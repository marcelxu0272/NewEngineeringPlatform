...
// 新增独立的图表组件
const MonthlyChart = ({ chartData, selectedMetric, metrics, formatNumber }: { chartData: any[], selectedMetric: string, metrics: any[], formatNumber: (num: number) => string }) => {
  useEffect(() => {
    const options = {
      chart: {
        type: 'column'
      ...
      yAxis: [{
        title: {
          text: metrics.find(m => m.id === selectedMetric)?.name
        },
        labels: {
      ...
      series: [{
        ...
        name: '当年累计',
        type: 'line',
        data: chartData.map(d => d[`${selectedMetric}Cumulative`]),
        yAxis: 1,
        color: '#85e5a7',
        lineWidth: 5
      }],
      tooltip: {
        formatter: function() {
          return `${this.x}<br/>${this.series.name}: ${formatNumber(this.y)}`;
        }
      },
      credits: {
        enabled: false
    ...
    const chart = Highcharts.chart('monthlyChart', options);
  ...
  }, [chartData, selectedMetric, metrics, formatNumber]);

  return <div id="monthlyChart" style={{width: '100%', minHeight: '400px'}} />;
};
...
const DataDashboard = () => {
    ...
    if (numericChange > 0) {
      return (
        <div className="flex items-center">
          <ArrowUpOutlined className="w-3 h-3 text-red-500 mr-0.5" />
      ...
      return (
        <div className="flex items-center">
          <ArrowDownOutlined className="w-3 h-3 text-green-500 mr-0.5" />
  ...
  useEffect(() => {
    if (selectedSubmenu === 'sectorCost') {
      const options = {
        ...
        xAxis: {
          categories: projectCostData.map(item => item.month)
        },
        yAxis: {
          title: {
            text: '金额'
        ...
        tooltip: {
          formatter: function() {
            return `${this.x}<br/>${this.series.name}: ${formatNumber(this.y)}<br/>` +
                   `总计: ${formatNumber(this.point.stackTotal)}`;
          }
        },
        plotOptions: {
          column: {
        ...
        series: [{
          name: '差旅及报销',
          data: projectCostData.map(item => item.差旅及报销),
          color: '#007069'
        }, {
          name: '人力成本',
          data: projectCostData.map(item => item.人力成本),
          color: '#007069cc'
        }, {
          name: '低负荷成本',
          data: projectCostData.map(item => item.低负荷成本),
          color: '#00706999'
        }, {
          name: '一般付款',
          data: projectCostData.map(item => item.一般付款),
          color: '#007069aa'
        }, {
          name: '涉外支付',
          data: projectCostData.map(item => item.涉外支付),
          color: '#007069bb'
        }, {
          name: '设计服务分包',
          data: projectCostData.map(item => item.设计服务分包),
          color: '#007069cc'
        }, {
          name: '采购',
          data: projectCostData.map(item => item.采购),
          color: '#007069dd'
        }, {
          name: '施工分包',
          data: projectCostData.map(item => item.施工分包),
          color: '#007069ee'
        }, {
          name: '其他',
          data: projectCostData.map(item => item.其他),
          color: '#007069ff'
        }],
        credits: {
          enabled: false
      ...
      const chart = Highcharts.chart('projectCostChart', options);
  ...
  }, [selectedSubmenu, projectCostData]);

  // 修改renderProjectCostChart只返回JSX
  const renderProjectCostChart = () => (
    <Card className="p-4 mb-4">
      <h3 className="text-lg font-semibold mb-2">项目成本月分布</h3>
      <div id="projectCostChart" style={{width: '100%', minHeight: '400px'}} />
    </Card>
  );
  ...
  return (
    <div className="flex bg-[#f8faff]">
      ...
      {/* 主要内容区域 */}
      <div className="flex ml-16 w-full bg-[#f8faff]">
        {/* 可折叠的二级目录 */}
        <div className={`${
          isSidebarExpanded ? 'w-1/5' : 'w-8'
        } min-h-screen border-r transition-all duration-300 relative bg-white`}>
          {/* 添加折叠按钮 */}
          <button
          ...
          {/* 筛选内容 - 添加上边距以避免被顶部遮挡 */}
          <div className={`${isSidebarExpanded ? 'opacity-100' : 'opacity-0 invisible'} transition-opacity duration-300 h-full w-full pt-24 px-6`}>
            <Button 
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex-1 h-3 bg-[#007069]/20 rounded-full overflow-hidden">
                          <div 
                            ...
                            style={{ width: `${metricInfo.completionRate}%` }}
                          />
                        </div>
                        <span className="text-lg font-semibold text-[#007069] ml-8">{metricInfo.completionRate}%</span>
                      </div>
                ...
                <div className="flex mt-4 h-[500px]">
                  <Card className="p-4 mb-4 flex-grow-[2] flex flex-col">
                    ...
                    <div className="flex-grow overflow-hidden">
                      <MonthlyChart chartData={chartData} selectedMetric={selectedMetric} metrics={metrics} formatNumber={formatNumber} />
                    </div>
                  </Card>
