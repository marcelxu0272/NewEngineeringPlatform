...
import { ClearOutlined, SettingOutlined, PlusOutlined, MinusCircleOutlined, UploadOutlined, ExpandAltOutlined, ShrinkOutlined, SearchOutlined, ArrowLeftOutlined } from '@ant-design/icons';
...
const TableHeader: React.FC = () => {
  return (
    <div className="bg-gray-100 rounded-lg p-4 mb-4 font-semibold">
...
function getSkillLevel(level: number): string {
  const levelMap = {
    ...
    3: 'bg-[#9e4795] text-white border-[#9e4795]'
  }
...
const PersonCard: React.FC<{ person: PersonCard; checkedSkills: string[]; onClick: () => void }> = ({ person, checkedSkills, onClick }) => {
  ...
  return (
    ...
    >
      {/* 第一行：基本信息 */}
      <div className="grid grid-cols-6 gap-4 bg-gray-100 p-2 pl-4 rounded-tl-lg rounded-tr-lg">
        ...
        <div className="flex items-center">
          <p className="text-gray-600">{person.region}</p>
        </div>
        <div className="flex items-center">
          <p className="text-gray-600">{formatSpecialty(person.specialty)}</p>
        </div>
      </div>
